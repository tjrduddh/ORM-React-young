...
const DataBindingTest = () => {
  ...
  //메시지 입력UI요소의 입력값이 변경될때마다 입력 이벤트를 처리해주는 이벤트 핸들러 함수
  const handleMessage = (e) => {
    //이벤트가 발생한 입력요소를 찾고 입력요소의 현재 입력값을 추출해서 콘솔에 출력해봅니다.
    console.log("사용자 입력 이벤트가 발생했습니다.", e.target.value);
  ...
  //message 데이터 스테이트값을 초기화해서 바인딩되어 있는 각종 UI의 값을 초기화 처리한다
  const handleInit = () => {
    setMessage("");
  };
  ...
  return (
    <div>
      ...
      텍스트박스에 입력값이 바뀔때마다 바인딩된 데이터소스(해당 DATA State)를 변경해줘야 합니다 -양방향 데이터바인딩 */}
      <input
        type="text"
        name="message"
