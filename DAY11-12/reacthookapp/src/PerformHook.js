...
const PerformHook = () => {
  ...
  //useMemo(실행할 함수,[데이터변경조건상태값지정])
  //list 숫자배령의 값이 변경될때만 getAverage() 함수를 호출하게 한다.
  //메모제이션 기능을 이용해 list의 값을 별도 저장소에 기록하고 있다가 변경을 감지하면 getAverage()함수를 실행시킨다
  //불필요한 화면 렌더링시마다 불필요하게 getAverage()함수를 실행시키는 불합리를 방지하여 성능을 향상시킨다.
  //useMemo는 특정 재사용가능함수를 특정조건애서 실행할 수 있게하여 리액트앱 성능을 담보시켜준다.
  ...
  //이벤트 처리함수와 관련된 특정 데이터가 변경이 발생할때만 해당 이벤트를 핸들러 함수가 생성되게하면
  ...
  //   const onAdd = () => {
  //     //array.concat() 기존 배열에 신규데이터를 추가해주고 새로운 복사본을 만들어준다(DeepCopy)
  //     setList(list.concat(parseInt(number)));
  //     setNumber(0);
  //   };

  //useCallback 훅을 통해서 onAdd함수는 number,list데이터가 변경될때만 해당 함수가 만들어지게 하여
  //불필요하게 모든 이벤트 또는 화면렌더링시 함수가 만들어지는 것을 방지하여 성능을 개선합니다.
