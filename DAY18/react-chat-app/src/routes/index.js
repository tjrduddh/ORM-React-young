...
//해당 컨텐츠 페이지를 감싸주는 인증이 필요한 레이아웃 컴포넌트와 비인증 레이아웃 컴포넌트를
//라우팅 페이지 컴포넌트와 통합해서 최종 브라우저에서 렌더링 시키는 역할 제공
...
//routes.js 모듈에서 정의한 화면별 라우팅 규칙 참조
import { authProtectedRoutes, publicRoutes } from "./routes";

//비인증 기반 레이아웃 화면 컴포넌트 참조
import NonAuthLayput from "../layouts/NonAuthLayput";

//인증 기반 레이아웃 화면 컴포넌트 참조
import AuthLayout from "../layouts/AuthLayout";

//로그인 여부를 체크한 후에 인증되었을때만 보여줘야하는 주소를 호출한 경우 로그인 페이지로 반환하는 함수 정의
const AuthProtected = (props) => {
  //로그인한 사용자 JWT토큰이 없거나 현재 호출하는 페이지가 인증에 의해 보호되는 페이지인 경우 로그인페이지로
...
const Routes = () => {
  return (
    <React.Fragment>
      {/* Suspense를 이용해 해당 태그내에 정의된 모든 개별 라우팅 화면컴포넌트가 완전 렌더링 전에 임시로 보여줄
        UI요소를 fallback속성으로 정의한다 */}
      <Suspense fallback={<div>로딩중...</div>}>
        ...
        <SwitchRoute>
          ...
          {publicRoutes.map((route, idx) => (
            <Route
              path={route.path}
              layout={NonAuthLayput}
              element={<NonAuthLayput>{route.component}</NonAuthLayput>}
              key={idx}
              isAuthProtected={false}
            />
          ))}

          {/* 인증이 반드시 필요한 페이지 라우팅 컴포넌트에 대한 레이아웃 통합 적용하기
          각각의 페이지 컴포넌트를 로그인여부/인증여부를 체크해 비로그인인경우 로그인 페이지로 이동처리하는 로직추가적용 */}
